...
class PatientUpdate(BaseModel):
    ...
    height: Annotated[Optional[float], Field(default=None, gt=0, description="height of the patient in mts")]
...
@app.get("/")
def hello():
    return {'message': 'Patient Management System for Doctors API'}

@app.get("/about")
def about():
    return {'message': 'A fully functional API to manage your patient records specially designed for doctors'}
...
def create_patient(patient: Patient):
    ...
    if patient.id in data:
        raise HTTPException(status_code=400,detail='Patient already exixts in database')
...
def update_patient(patient_id: str, patient_update: PatientUpdate):
    ...
    upadted_patient_info=patient_update.model_dump(exclude_unset=True)

    for key, value in upadted_patient_info.items():
